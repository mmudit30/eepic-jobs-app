...
import { StyleSheet, Text, View, ActivityIndicator, ViewBase } from 'react-native';
import * as firebase from 'firebase';
import { ScrollView, FlatList, TouchableOpacity } from 'react-native-gesture-handler';
require("firebase/firestore");
import {Ionicons} from '@expo/vector-icons';

export default class JobsAppliedScreen extends React.Component  {
state={
      searchStatus: '',
      jobsApplied: [''],
      jobs:[{
...
componentDidMount() {
    ...
    this.setState({ searchStatus: 'searching' });
        firebase.firestore().collection('users')
            ...
            .get().then((userData)=>{
                let arrJobsApplied=[];
                arrJobsApplied = userData.data().jobsApplied;
                if(arrJobsApplied.length != 0){
                    let JobDataArr=[];
                    arrJobsApplied.forEach(JobID =>{
                        firebase.firestore().collection('jobs').doc(JobID).get()
                            .then((jobData)=>{
                                JobDataArr.push(jobData.data());
                                // setJobs(JobDataArr);
                            }).then(()=>{
                                if(JobDataArr.length == arrJobsApplied.length){
  ...
  render(){
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={{color: 'rgb(0,100,200)',fontSize: 18, alignSelf: "center", textTransform: "uppercase"}} onPress={()=> props.navigation.goBack()}>Applied Jobs</Text>
        {
